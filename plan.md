...
```bash
node scripts/benchmark-fill-grid.js --model gpt-5.4 --tasks-dir tasks/fill-grid/manual --minEntryLength 3 --count 5
```

说明：

- `--model <name>`: 只运行 `models/<name>/index.js`
- `--tasks-dir <dir>`: 指定要跑的 task 目录，支持递归读取
