...
- word: 词条本体
- reading: 原始读音，保留词库中的自然假名写法
- normalizedReading: 实际填入格子的规范化假名串；小字按大字处理
- clue: 题目提示
- pos: 词性
- level: 等级信息，例如 N5、N4
- tags: 主题标签数组
...
- 小字假名统一按大字处理，例如 `ゃ -> や`、`っ -> つ`
- 如果实现内部使用规范化后的入格串，必须在结果中保留原始 `reading`，并显式输出 `normalizedReading`
